...
def virtual(virtualname, opts, serves):
    '''
    Only load if the desired provider module is present and gitfs is enabled
    properly in the master config file.
    '''
    if not virtualname in opts[serves+'_fileserver_backend']:
        return False
    try:
        return virtualname if _get_provider(opts) else False
    except SaltException as exc:
        log.error(exc)
        return False

def _verify_gitpython(opts, quiet=False):
    '''
...
def _verify_pygit2(opts, quiet=False):
    '''
...
def _verify_dulwich(opts, quiet=False):
    '''
...
def _get_provider(opts):
    ...
    if not provider:
        ...
        if _verify_gitpython(opts, quiet=True):
            return 'gitpython'
        elif _verify_pygit2(opts, quiet=True):
            return 'pygit2'
        elif _verify_dulwich(opts, quiet=True):
            return 'dulwich'
        else:
            log.error(
        ...
        elif provider == 'gitpython' and _verify_gitpython(opts):
            return 'gitpython'
        elif provider == 'pygit2' and _verify_pygit2(opts):
            return 'pygit2'
        elif provider == 'dulwich' and _verify_dulwich(opts):
            return 'dulwich'
    return ''
...
class GitFSB(salt.fileserver.FileserverBackend):

    # need to cache init, get provider, etc. calls in the singleton
    def __init__(self, opts):
        super(GitFSB, self).__init__(opts)

    def popts(self, key, default=''):
        key = self.__class__.serves + '_' + key
    ...
    def init(self):
        ...
        '''
        bp_ = os.path.join(self.opts['cachedir'], self.__class__.serves + '_gitfs')
        ...
        ssl_verify = 'true' if self.opts.get('gitfs_ssl_verify', True) else 'false'
        repos = []
        for _, opt in enumerate(self.popts('gitfs_remotes')):
            if provider == 'pygit2':
                ...
                if provider == 'gitpython':
                    ...
                    if not os.listdir(rp_):
                        try:
                            ...
                            conf = self.__class__._dulwich_conf(repo)
    ...
    def update(self):
        ...
        env_cache = os.path.join(self.opts['cachedir'], self.__class__.serves +
                '_gitfs/envs.p')
        ...
        try:
            salt.fileserver.reap_fileserver_cache_dir(
                os.path.join(self.opts['cachedir'], self.__class__.serves
                    + 'gitfs/hash'), self.find_file)
        except (IOError, OSError):
    ...
    def envs(self, ignore_cache=False):
        ...
        if not ignore_cache:
            env_cache = os.path.join(self.opts['cachedir'], self.__class__.serves
                    + '_gitfs/envs.p')
            cache_match = salt.fileserver.check_env_cache(self.opts, env_cache)
            if cache_match is not None:
                return cache_match
        base_branch = self.popts('gitfs_base')
        provider = _get_provider(self.opts)
        ret = set()
        repos = self.init()
        for repo in repos:
    ...
    def find_file(self, path, short='base', **kwargs):
        '''
        ...
        local_path = path
        if self.popts('gitfs_root'):
            path = os.path.join(self.popts('gitfs_root'), local_path)

        if short == 'base':
            short = base_branch
        dest = os.path.join(self.opts['cachedir'], self.__class__.serves +
                '_gitfs/refs', short, path)
        hashes_glob = os.path.join(self.opts['cachedir'],
                                   self.__class__.serves + '_gitfs/hash',
                                   short,
                                   '{0}.hash.*'.format(path))
        blobshadest = os.path.join(self.opts['cachedir'],
                                   self.__class__.serves + '_gitfs/hash',
                                   short,
    ...
    def file_hash(self, load, fnd):
        ...
        path = fnd['path']
        if self.popts('gitfs_root'):
            relpath = os.path.join(self.popts('gitfs_root'), relpath)
            path = os.path.join(self.popts('gitfs_root'), path)

        hashdest = os.path.join(self.opts['cachedir'],
                                self.__class__.serves + '_gitfs/hash',
                                short,
    ...
    def _get_file_list(self, load):
        ...
        base_branch = self.popts('gitfs_base')
        gitfs_root = self.popts('gitfs_root')
        provider = _get_provider(self.opts)
    ...
    def file_list_emptydirs(self, load):
        ...
        '''
        return self._file_lists(load, 'empty_dirs')


    def _get_file_list_emptydirs(self, load):
        '''
        Return a list of all empty directories on the master
        '''
        if 'env' in load:
