...
class RootsFSB(salt.fileserver.FileserverBackend):
    
    def __init__(self, opts):
        super(RootsFSB, self).__init__(opts)
        self.roots_dir = {
                'states': 'file_roots',
                'pillar': 'pillar_roots'
    ...
    def update(self):
        ...
        try:
            salt.fileserver.reap_fileserver_cache_dir(
                os.path.join(self.opts['cachedir'], 'roots/hash'),
                self.find_file
            )
        ...
        new_mtime_map = salt.fileserver.generate_mtime_map(self.opts[self.roots_dir])
    ...
    def envs(self):
        ...
        '''
        return self.opts[self.roots_dir].keys()

    def find_file(self, path, saltenv='base', env=None, **kwargs):
        '''
        ...
        if os.path.isabs(path):
            return fnd
        if saltenv not in self.opts[self.roots_dir]:
            return fnd
        if 'index' in kwargs:
            try:
                root = self.opts[self.roots_dir][saltenv][int(kwargs['index'])]
            except IndexError:
            ...
            if os.path.isfile(full) and not salt.fileserver.is_file_ignored(self.opts, full):
                fnd['path'] = full
                fnd['rel'] = path
            return fnd
        for root in self.opts[self.roots_dir][saltenv]:
            full = os.path.join(root, path)
            if os.path.isfile(full) and not salt.fileserver.is_file_ignored(self.opts, full):
                fnd['path'] = full
                fnd['rel'] = path
    ...
    def file_hash(self, load, fnd):
        ...
        cache_path = os.path.join(self.opts['cachedir'],
                                  'roots/hash',
                                  load['saltenv'],
                ...
                with salt.utils.fopen(cache_path, 'rb') as fp_:
                    try:
                        ...
                        try:
                            ...
                            pass
                        return self.file_hash(load, fnd)
    ...
    def _file_lists(self, load, form):
        ...
        if 'env' in load:
            ...
            load['saltenv'] = load.pop('env')
        if load['saltenv'] not in self.opts[self.roots_dir]:
            return []

        list_cachedir = os.path.join(self.opts['cachedir'], 'file_lists/' +
            self.__class__.serves + '_roots')
        ...
        if refresh_cache:
            ...
            for path in self.opts[self.roots_dir][load['saltenv']]:
                for root, dirs, files in os.walk(
                        path,
                        followlinks=self.opts['fileserver_followsymlinks']):
                    dir_rel_fn = os.path.relpath(root, path)
                    ret['dirs'].append(dir_rel_fn)
                    if len(dirs) == 0 and len(files) == 0:
                        if not salt.fileserver.is_file_ignored(self.opts, dir_rel_fn):
                            ret['empty_dirs'].append(dir_rel_fn)
                    for fname in files:
                        is_link = os.path.islink(os.path.join(root, fname))
                        if is_link:
                            ret['links'].append(fname)
                        if self.opts['fileserver_ignoresymlinks'] and is_link:
                            continue
                        rel_fn = os.path.relpath(
                                    os.path.join(root, fname),
                                    path
                                )
                        if not salt.fileserver.is_file_ignored(self.opts, rel_fn):
